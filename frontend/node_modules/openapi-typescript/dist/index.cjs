  ...
  for (const key of Object.keys(transformers)) {
    ...
    if (componentsObject[key]) {
      for (const [name, item] of getEntries(componentsObject[key], ctx)) {
        ...
        items.push(property);
        if (ctx.rootTypes) {
          let aliasName = pascalCase(singularizeComponentKey(key)) + pascalCase(name);
          let conflictCounter = 1;
          while (rootTypeAliases[aliasName] !== void 0) {
            conflictCounter++;
            aliasName = `${pascalCase(singularizeComponentKey(key))}${pascalCase(name)}_${conflictCounter}`;
          }
...
async function openapiTS(source, options = {}) {
  ...
  const ctx = {
    ...
    injectFooter: [],
    pathParamsAsTypes: (_p = options.pathParamsAsTypes) != null ? _p : false,
    postTransform: typeof options.postTransform === "function" ? options.postTransform : void 0,
    propertiesRequiredByDefault: (_q = options.propertiesRequiredByDefault) != null ? _q : false,
    redoc,
    silent: (_r = options.silent) != null ? _r : false,
    inject: (_s = options.inject) != null ? _s : void 0,
